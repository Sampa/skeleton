	...
	$.fn.xBreadcrumbs = function(settings){
		...
		function _build(){
			if(settings.collapsible){
				...
				element.children('LI').children(':first-child').css('white-space', 'nowrap').css('float', 'left');
				element.children('LI').children(':first-child').each(function(i, el){
					if(i != sz -1){
			...
			$("body").on('click','.breadCrumbLink', function(){
            ...
            element.children('LI').mouseenter(function(){
            	...
            	if(settings.collapsible && !$(this).hasClass('current')){
            		var initWidth = $(this).children(':first-child').attr('init-width');
            		$(this).children(':first-child').animate({width: initWidth}, 'normal');
            	}
            });
            ...
            if($(this).hasClass("last")){    
                ...
                if(settings.collapsible && !$(this).hasClass('current')){
                	$(this).children(':first-child').animate({width: settings.collapsedWidth}, 'fast');
                }
              }
            });
		};
